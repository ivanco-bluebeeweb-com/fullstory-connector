"""Connection lifecycle for FullStory Connector."""
...
@chat.function(
    "connect_fullstory",
    "Connect FullStory account via credentials.",
    action_type="write",
    chain_callable=True,
    event="fullstory-connector.connect_fullstory",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_fullstory(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect FullStory Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "FullStory Account",
...
@chat.function(
    "list_connections",
    "List connected FullStory accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_fullstory",
    "Disconnect FullStory account.",
    action_type="write",
    chain_callable=True,
    event="fullstory-connector.disconnect_fullstory",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_fullstory(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
