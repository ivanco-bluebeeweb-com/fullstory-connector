"""Resource handlers for FullStory Connector."""
...
@chat.function("list_events", "List events in FullStory.", action_type="read", chain_callable=True, event="fullstory-connector.list_events", effects=["read:events"], data_model=EventList)
...
@chat.function("get_event", "Get details of one Event in FullStory.", action_type="read", chain_callable=True, event="fullstory-connector.get_event", effects=["read:event"], data_model=EventRecord)
...
@chat.function("audit_event_health", "Audit health of FullStory events and connectivity.", action_type="read", chain_callable=True, event="fullstory-connector.audit_event_health", effects=["read:audit"], data_model=AuditHealthReport)
async def audit_event_health(params: ConnectionIdParams, ctx) -> ActionResult:
    ...
    try:
        ...
        return ActionResult.ok({
            ...
            "summary": f"FullStory healthy. Sampled {len(items)} events."
        }, summary=f"FullStory health check passed with {len(items)} events.")
    except Exception as e:
        return ActionResult.error(f"Error auditing FullStory health: {e}")
